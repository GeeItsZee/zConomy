...
public class Purge
{
    ...
    {
        if(!sender.hasPermission("zConomy.Purge"))
        {
            sender.sendMessage(Settings.format("NoPermission", "zConomy.Purge"));
            return;
        }

        zConomyDatabase database = plugin.getzConomyDatabase();

        Integer count = database.purgeAccounts();
        sender.sendMessage(Settings.format("AccountsPurged", count.toString()));
    }
}
