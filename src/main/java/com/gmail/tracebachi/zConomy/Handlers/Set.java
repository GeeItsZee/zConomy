...
public class Set
{
    ...
    {
        if(!sender.hasPermission("zConomy.Set"))
        {
            sender.sendMessage(Settings.format("NoPermission", "zConomy.Set"));
            return;
        }
        ...
        boolean found = database.updateBalance(name, amount);

        if(found)
        {
            sender.sendMessage(Settings.format("AccountSet", name, formatAmount(amount)));
        }
        else
        {
