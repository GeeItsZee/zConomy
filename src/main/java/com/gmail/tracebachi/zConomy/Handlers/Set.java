...
public class Set
{
  private static final String COMMAND_PERM = "zConomy.Set";

  private final zConomyPlugin plugin;
  private final Settings settings;

  public Set(zConomyPlugin plugin, Settings settings)
  {
    this.plugin = plugin;
  ...
  public void handle(CommandSender sender, String name, String amountString)
  {
    ...
    if (database.setBankAccountBalance(name, amount))
    {
      sender.sendMessage(settings.format("AccountSet", name, settings.formatAmount(amount)));
    }
    else
    {
