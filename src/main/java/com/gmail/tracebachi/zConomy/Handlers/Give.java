...
public class Give
{
    ...
    {
        if(!sender.hasPermission("zConomy.Give"))
        {
            sender.sendMessage(Settings.format("NoPermission", "zConomy.Give"));
            return;
        }
        ...
        if(found)
        {
            sender.sendMessage(Settings.format("AccountGive", formatAmount(amount), name));
        }
        else
        {
